...
def submit_assessment_answers():
    ...
    # Debug: Print the assessment data
    print(f"DEBUG: Assessment submission for emp_id={emp_id}, course={course_name}")
    ...
    for i, question in enumerate(questions):
        ...
        print(f"DEBUG: Question {i}: Correct={correct_answer}, User={user_answer}, Match={str(correct_answer) == str(user_answer)}")
    ...
    print(f"DEBUG: Final score: {score}/{total_questions} = {percentage_score}%")
    
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            # Debug: Print what we're inserting
            print(f"DEBUG: Inserting into assessment_marks: emp_id={emp_id}, course_name='{course_name}', marks_obtained={score}")
            ...
            cursor.execute("SELECT * FROM assessment_marks WHERE emp_id = %s AND course_name = %s ORDER BY assessment_id DESC LIMIT 1", (emp_id, course_name))
            inserted_record = cursor.fetchone()
            print(f"DEBUG: Inserted record: {inserted_record}")
